# Splitpoint — Software Box Prompt (Nano Banana 2)
...
FRONT PANEL (facing camera): bold large product name "Splitpoint" in clean modern sans-serif; below it the tagline "A/B testing and feature flags you own forever." in smaller white text; a central icon motif representing self-hosted experiments, a sub-3kb sdk and a real z-test — no per-visitor pricing; a small "Windows" platform badge near the bottom corner; a round orange price sticker badge reading "PAY ONCE — OWN FOREVER" with a large "$49" in the center. Small brand mark "OneTime Suite" at the bottom of the front panel.
...
"Experiments", "Goals two ways", "Tiny SDK", "React hook wrapper", "Results that mean something", "No Subscription. Ever.".
...
| Element | Exact text |
|---|---|
| Product name (front, largest) | Splitpoint |
| Tagline (front, under name) | A/B testing and feature flags you own forever. |
...
| Side panel bullet 1 | Experiments |
| Side panel bullet 2 | Goals two ways |
| Side panel bullet 3 | Tiny SDK |
| Side panel bullet 4 | React hook wrapper |
| Side panel bullet 5 | Results that mean something |
...
*Motif hint:* 🔀 Self-hosted experiments, a sub-3KB SDK and a real z-test — no per-visitor pricing.
*Replaces:* VWO ($199/mo)
