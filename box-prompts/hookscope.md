# Hookscope — Software Box Prompt (Nano Banana 2)
...
FRONT PANEL (facing camera): bold large product name "Hookscope" in clean modern sans-serif; below it the tagline "See every webhook. Replay any of them. Pay once." in smaller white text; a central icon motif representing self-hosted webhook capture, inspection, replay & forwarding — urls that never expire; a small "Windows" platform badge near the bottom corner; a round orange price sticker badge reading "PAY ONCE — OWN FOREVER" with a large "$24" in the center. Small brand mark "OneTime Suite" at the bottom of the front panel.
...
"Bins that accept anything", "Full capture", "Live inspector", "Replay", "Auto-forwarding", "No Subscription. Ever.".
...
| Element | Exact text |
|---|---|
| Product name (front, largest) | Hookscope |
| Tagline (front, under name) | See every webhook. Replay any of them. Pay once. |
...
| Side panel bullet 1 | Bins that accept anything |
| Side panel bullet 2 | Full capture |
| Side panel bullet 3 | Live inspector |
| Side panel bullet 4 | Replay |
| Side panel bullet 5 | Auto-forwarding |
...
*Motif hint:* 🪝 Self-hosted webhook capture, inspection, replay & forwarding — URLs that never expire.
*Replaces:* Pipedream ($29/mo)
