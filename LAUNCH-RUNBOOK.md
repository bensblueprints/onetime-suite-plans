...
6. **HOSTED MULTI-TENANT MODEL (final user decision 2026-07-16): we host everything; ONE container per app at `<slug>.onetimesuite.com`; users register/login with email+password.** One update to the app = rebuild + restart that container = all users get it. Implementation: `_shared/multi-tenant` module (build pending — see Notion "🛡 Production Hardening & Cost Plan"): register/login (bcrypt, httpOnly sessions), Whop purchase verification at signup (reuses whop-license OAuth), **per-user SQLite files** (`data/users/<id>.db`) so app queries stay unmodified and isolation stays file-based. Apps with background jobs (~15–20: cron/price/uptime monitors, email senders) need their schedulers looping over all user DBs — hand-verify those. Schema updates must be additive boot-time migrations, applied per user-DB on open. Web apps cannot flip to "Available now" until their multi-tenant rollout is done and verified. Self-hosting via `npm run activate` remains available for DIY buyers.
